...
*cc(n)..   sum(f, char("mean",n,f)*x(f)) - 1.96*sqrt(sum(f, char("variance",n,f)*sqr(x(f)))) =g= req(n);
cc(n)..   sum(f, char("mean",n,f)*x(f)) - 3.0*sqrt(sum(f, char("variance",n,f)*sqr(x(f)))) =g= req(n);
